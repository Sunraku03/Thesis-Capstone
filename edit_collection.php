    ...
    switch ($section) {
        ...
        case 'edit_net_payment':
            ...
            case 'edit_collection':
                for ($i = 1; $i <= 6; $i++) {
                    $fields["payment_$i"] = $_POST["payment_$i"];
                    $fields["payment_{$i}_date"] = $_POST["payment_{$i}_date"];
                    $fields["payment_{$i}_method"] = $_POST["payment_{$i}_method"];
                }
                $fields['payment_status'] = $_POST['payment_status'];
                $fields['end_date'] = $_POST['end_date'];
                $fields['ctpl'] = $_POST['ctpl'];
                $fields['bank_status'] = $_POST['bank_status'];
                ...
                for ($i = 1; $i <= 6; $i++) {
                    if (isset($_FILES["proof_payment_$i"]) && $_FILES["proof_payment_$i"]['error'] === UPLOAD_ERR_OK) {
                        $fileTmpPath = $_FILES["proof_payment_$i"]['tmp_name'];
                        $fileName = $_FILES["proof_payment_$i"]['name'];
                        $fileExtension = strtolower(pathinfo($fileName, PATHINFO_EXTENSION));

                        if (in_array($fileExtension, $allowedTypes)) {
                            $newFileName = uniqid("proof_{$i}_") . '.' . $fileExtension;
                            $dest_path = $uploadDir . $newFileName;

                            if (move_uploaded_file($fileTmpPath, $dest_path)) {
                                $fields["proof_payment_$i"] = $newFileName;
                            }
                        }
                    }
                }
    ...
    <form method="post" enctype="multipart/form-data">
        <?php switch ($section):
            ...
            case 'edit_vehicle': ?>
                ...
                <input type="date" name="date_remittance" value="<?= $client['date_remittance'] ?>" class="form-control mb-3">
            ...
            case 'edit_collection':
                for ($i = 1; $i <= 6; $i++): ?>
                    <h5 class="mt-3">Payment <?= $i ?></h5>
                    <input type="text" name="payment_<?= $i ?>" value="<?= htmlspecialchars($client["payment_$i"]) ?>" class="form-control mb-2" placeholder="Payment <?= $i ?> Amount">
                    <input type="date" name="payment_<?= $i ?>_date" value="<?= htmlspecialchars($client["payment_{$i}_date"]) ?>" class="form-control mb-2" placeholder="Payment <?= $i ?> Date">
                    <input type="text" name="payment_<?= $i ?>_method" value="<?= htmlspecialchars($client["payment_{$i}_method"]) ?>" class="form-control mb-3" placeholder="Payment <?= $i ?> Method">
                    <input type="file" name="proof_payment_<?= $i ?>" accept="image/*" class="form-control mb-3">
                    <?php if (!empty($client["proof_payment_$i"])): ?>
                        <img src="uploads/<?= htmlspecialchars($client["proof_payment_$i"]) ?>" alt="Proof of Payment <?= $i ?>" style="max-height: 100px; border-radius: 6px;">
                    <?php endif; ?>
                <?php endfor; ?>

                <input type="text" name="payment_status" value="<?= htmlspecialchars($client['payment_status']) ?>" class="form-control mb-3" placeholder="Payment Status">
                <input type="date" name="end_date" value="<?= $client['end_date'] ?>" class="form-control mb-3">
                <input type="text" name="ctpl" value="<?= htmlspecialchars($client['ctpl']) ?>" class="form-control mb-3" placeholder="CTPL">
                <input type="text" name="bank_status" value="<?= htmlspecialchars($client['bank_status']) ?>" class="form-control mb-3" placeholder="Bank Status">
                <input type="text" name="remarks" value="<?= htmlspecialchars($client['remarks']) ?>" class="form-control mb-3" placeholder="Remarks">
            <?php break; 
        endswitch; ?>
